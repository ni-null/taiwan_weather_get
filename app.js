...
function get_city_weather(url) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const last_update_time = await config_table.get_city_upadte_time_from_conf(city_name_che)
    ...
    const update_time_result = await config_table.upadte_data_time(update_time, city_name_che)
...
async function get_taiwan_weather(api_key) {
  ...
  const url =
    "https://opendata.cwb.gov.tw/fileapi/v1/opendataapi/F-D0047-091?Authorization=" +
    api_key +
    "&downloadType=WEB&format=JSON";
  ...
  return new Promise(async (resolve, reject) => {
    ...
    //檢查更新時間，一致則返回
    const update_time = responsed.datasetInfo.update
    const last_update_time = await config_table.get_city_upadte_time_from_conf(name_che)
    if (last_update_time.update_time != update_time) console.log("更新天氣資料")
    else {
      setTimeout(() => { resolve() }, 200)
      return
    ...
    const update_time_result = await config_table.upadte_data_time(update_time, '台灣')
